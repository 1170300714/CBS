...
CFG=vit_b16_kmeans_random_discard_greedy_add_pseudo
TRAINER=CoOp_CIFAR_wo_Base_Our_ACIL_Distribution
R=$1
CLASS_PER_TASK=20
for ((i = 0 ; i < 5 ; i++))
do
    if [ $i -eq 0 ]; then
        python train.py \
            ...
            AL.SAMPLE_SAVE_PATH CIFAR_100/acil/distribution_kmeans_random_discard_greedy_add_pseudo/R_${R}/SEED1 \
            AL.ROUND ${R}
    else
        j=$(($i-1))
        python train.py \
