...
class CoOp_CIFAR_wo_Base_Our_ACIL_Exist_AL(TrainerX):
    ...
    def after_round(self, epoch):
        ...
        if do_test:
            if self.cfg.TEST.FINAL_MODEL == "best_val":
                ...
                self.load_model(self.output_dir, epoch)
            else:
        ...
        for R in range(ROUND):
            ...
            if self.cfg.AL.PCB and R != 0:  # 默认第一回合都是random
                ...
                if ROUND==1:
                    ...
                    al_method = self.cfg.AL.NAME if R != 0 or self.cfg.AL.NAME == 'random_force_balance' else 'random'# 默认第一回合都是random
                cl_method = self.cfg.CL.NAME
                assert hasattr(self.train_set, 'idxs_lb'), "The dataset is not modified for Active learning."
                active_alg = get_strategy(al_method, self.train_set, self.model, self.device, self.cfg, self.tfm_train,self.tfm_test, select_data_save_path, cl_method)
            ...
            if R != ROUND - 1:
                self.after_round(epoch = self.max_epoch)
