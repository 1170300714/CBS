...
class MiniImageNet_CL(TorchDataset):

    def __init__(self,shot, tfm,task_id,mode,class_per_task):
        ...
        self.shot = shot
        self.task_len = int(100/class_per_task)
        ...
        if mode=='train':
            ...
            for i in range(len(self.images_list)):
                ...
                label = self.labeled_list[i]
                if label in self.class_idx_dict:
                    self.class_idx_dict[label].append(image_path)
            ...
            for c in select_class_id:
                image_path_list = self.class_idx_dict[c]

                for id in image_path_list:
                    self.images_list.append(id)
    ...
    def __getitem__(self, i):
        ...
        path, targets = self.images_list[i], self.labeled_list[i]
...
class CoOp_miniImageNet_wo_Base_Our_ACIL_Exist_AL(TrainerX):
    ...
    def build_data_loader(self):
        ...
        train_set_task0 = MiniImageNet_CL(shot=self.cfg.DATASET.NUM_SHOTS,tfm=self.tfm_train,class_per_task=20, task_id = task_id_now,mode='train')
        ...
        test_set_task0 = MiniImageNet_CL(shot=self.cfg.DATASET.NUM_SHOTS,tfm=self.tfm_test,class_per_task=20,task_id = task_id_now,mode='test')
    ...
    def build_model(self):
        ...
        self.class_per_task = class_per_task
        task_num = int(100/class_per_task)
        ...
        self.classnames_all = ['house_finch', 'robin', 'triceratops', 'green_mamba', 'harvestman', 'toucan', 'goose', 'jellyfish', 'nematode', 'king_crab', 'dugong', 'Walker_hound', 'Ibizan_hound', 'Saluki', 'golden_retriever', 'Gordon_setter', 'komondor', 'boxer', 'Tibetan_mastiff', 'French_bulldog', 'malamute', 'dalmatian', 'Newfoundland', 'miniature_poodle', 'white_wolf', 'African_hunting_dog', 'Arctic_fox', 'lion', 'meerkat', 'ladybug', 'rhinoceros_beetle', 'ant', 'black-footed_ferret', 'three-toed_sloth', 'rock_beauty', 'aircraft_carrier', 'ashcan', 'barrel', 'beer_bottle', 'bookshop', 'cannon', 'carousel', 'carton', 'catamaran', 'chime', 'clog', 'cocktail_shaker', 'combination_lock', 'crate', 'cuirass', 'dishrag', 'dome', 'electric_guitar', 'file', 'fire_screen', 'frying_pan', 'garbage_truck', 'hair_slide', 'holster', 'horizontal_bar', 'hourglass', 'iPod', 'lipstick', 'miniskirt', 'missile', 'mixing_bowl', 'oboe', 'organ', 'parallel_bars', 'pencil_box', 'photocopier', 'poncho', 'prayer_rug', 'reel', 'school_bus', 'scoreboard', 'slot', 'snorkel', 'solar_dish', 'spider_web', 'stage', 'tank', 'theater_curtain', 'tile_roof', 'tobacco_shop', 'unicycle', 'upright', 'vase', 'wok', 'worm_fence', 'yawl', 'street_sign', 'consomme', 'trifle', 'hotdog', 'orange', 'cliff', 'coral_reef', 'bolete', 'ear']
    ...
    def train(self):
        ...
        for R in range(ROUND):
            # 利用现有的CLIP模型来筛选sample
            if self.cfg.AL.PCB and R != 0:  # 默认第一回合都是random
                budget = int(np.sum(~self.dm.dataset.idxs_lb).item() * self.cfg.AL.YITA)  # 未标注数据的YITA%来预标注
                al_method = self.cfg.AL.NAME
                assert hasattr(self.dm.dataset, 'idxs_lb'), "The dataset is not modified for Active learning."
                active_alg = get_strategy(al_method, self.dm.dataset, self.model, self.device, self.cfg)
                idxs_active = active_alg.query(budget, R)

                # PCB
                pcb_alg = get_strategy('PCB', self.dm.dataset, self.model, self.device, self.cfg)
                idxs_active = pcb_alg.query(self.dm.num_classes, idxs_active)

                active_alg.update(idxs_active)
                self.dm.dataset.idxs_lb = active_alg.dset.idxs_lb  # 其实应该是不需要的，为了以防万一，更新一下
                ...
                if ROUND==1:
                    ...
                    al_method = self.cfg.AL.NAME if R != 0 or self.cfg.AL.NAME == 'random_force_balance' else 'random'
                ...
                print("begin actively select sample in round {}.".format(R))
                idxs_active = active_alg.query(budget, R)
                active_alg.update(idxs_active)
            ...
            print("Begin round {} of task {}.".format(R,self.task_id))
    ...
    def after_epoch(self):
        ...
        if do_test and self.cfg.TEST.FINAL_MODEL == "best_val" and self.epoch%2==0:
