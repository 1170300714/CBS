...
class CoOp_CIFAR_wo_Base_Our_ACIL_Distribution(TrainerX):
    ...
    def write_selected_sample(self,active_alg,task_id,save_path):
        ...
        label_list = [self.train_set.data[ind][1] for ind in np.where(active_alg.dset.idxs_lb)[0].tolist()]
    ...
    def write_unselected_sample(self,active_alg,task_id,save_path):
        ...
        save_file = os.path.join(save_path,'session_{}_unlabeled.txt.txt'.format(task_id))

        image_path_list = [ind for ind in np.where(active_alg.dset.idxs_lb==0)[0].tolist()]
        label_list = [self.train_set.data[ind][1] for ind in np.where(active_alg.dset.idxs_lb==0)[0].tolist()]
        with open(save_file,'w') as f:
            for i in range(len(image_path_list)):
                f.write(str(image_path_list[i])+" "+str(label_list[i])+"\n")
    ...
    def train(self):
        ...
        GD = self.generate_GD(use_pseudo_label=use_pseudo)
        if self.task_id>0:
    ...
    def generate_pseudo(self, ):
        idx_list = [ind for ind in np.where(self.train_set.idxs_lb==0)[0].tolist()]
    ...
    def generate_GD(self,use_pseudo_label=False):
        ...
        if use_pseudo_label:
            pseudo_feats = self.generate_pseudo()
